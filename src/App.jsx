...
function Ticker() {
  return (
    <div style={{ background: C.navy, padding: "10px 0", overflow: "hidden" }}>
      <div style={{ display: "inline-block", whiteSpace: "nowrap", animation: "ticker 60s linear infinite" }}>
        {[...NEWS_ITEMS, ...NEWS_ITEMS].map((item, i) => (
          <span key={i} style={{ fontFamily: "'IBM Plex Mono', monospace", fontSize: 11, color: C.green, marginRight: 64 }}>{item}</span>
        ))}
      </div>
    </div>
  );
...
function Navbar({ scrollTo }) {
  ...
  return (
    <nav style={{ position: "sticky", top: 0, zIndex: 100, background: scrolled ? "rgba(255,255,255,0.97)" : C.white, borderBottom: `1px solid ${C.greyBorder}`, backdropFilter: "blur(10px)", boxShadow: scrolled ? "0 2px 20px rgba(26,39,68,0.08)" : "none", transition: "all 0.3s" }}>
      <div style={{ maxWidth: 1200, margin: "0 auto", padding: "0 32px", height: 64, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 28 }}>
          ...
          <a href="sms:8018222706" style={{ background: C.teal, color: C.white, padding: "9px 20px", fontSize: 12, fontWeight: 700, textDecoration: "none", borderRadius: 4, letterSpacing: "0.04em" }}>Text for Demo</a>
        </div>
      </div>
    </nav>
  );
      ...
      <div style={{ maxWidth: 1200, margin: "0 auto" }}>
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 80, alignItems: "center" }}>
          <div>
            ...
            <p style={{ fontSize: 16, lineHeight: 1.75, color: C.muted, marginBottom: 32, maxWidth: 480 }}>
            ...
            <div style={{ display: "flex", gap: 14, flexWrap: "wrap" }}>
              <button onClick={() => scrollTo("Scorecard")} style={{ background: C.navy, color: C.white, border: "none", padding: "14px 28px", fontSize: 13, fontWeight: 700, cursor: "pointer", borderRadius: 4, letterSpacing: "0.04em" }}>Check Your Risk Score</button>
              <a href="sms:8018222706" style={{ background: C.white, color: C.teal, border: `2px solid ${C.teal}`, padding: "12px 28px", fontSize: 13, fontWeight: 700, cursor: "pointer", borderRadius: 4, textDecoration: "none", letterSpacing: "0.04em" }}>📱 Text 801-822-2706</a>
            </div>
          </div>
          <div>
            <div style={{ background: C.grey, border: `1px solid ${C.greyBorder}`, borderRadius: 8, padding: 32 }}>
              <div style={{ fontSize: 10, fontWeight: 700, letterSpacing: "0.18em", textTransform: "uppercase", color: C.muted, marginBottom: 20 }}>Time Until First Deadline — March 15, 2026</div>
    ...
    <section style={{ background: C.white, padding: "72px 32px", borderBottom: `1px solid ${C.greyBorder}` }}>
      <div style={{ maxWidth: 1200, margin: "0 auto" }}>
        <div style={{ marginBottom: 48 }}>
          ...
          <h2 style={{ fontFamily: "'DM Serif Display', serif", fontSize: 38, fontWeight: 400, color: C.navy, letterSpacing: "-0.02em" }}>The phased reduction timeline.</h2>
        </div>
...
const QUESTIONS = [
  { q: "How do you currently track certificate expiration?", options: [{ label: "Automated CLM platform", score: 0 }, { label: "Calendar reminders / email alerts", score: 2 }, { label: "Spreadsheet or shared doc", score: 3 }, { label: "We don't track them formally", score: 4 }] },
  { q: "How many certificates does your organization manage?", options: [{ label: "Fewer than 25", score: 0 }, { label: "25–100", score: 1 }, { label: "100–500", score: 2 }, { label: "500+", score: 3 }] },
  { q: "Have you experienced an unplanned outage caused by an expired certificate?", options: [{ label: "Never", score: 0 }, { label: "Once, years ago", score: 1 }, { label: "Once in the past 2 years", score: 3 }, { label: "More than once", score: 4 }] },
  { q: "What environments do your certificates span?", options: [{ label: "Single on-prem environment", score: 0 }, { label: "Cloud only (single provider)", score: 1 }, { label: "Hybrid or multi-cloud", score: 2 }, { label: "Multi-cloud + on-prem + edge", score: 3 }] },
  { q: "Are you subject to compliance frameworks that require certificate management?", options: [{ label: "No compliance requirements", score: 0 }, { label: "SOC 2 only", score: 1 }, { label: "PCI, HIPAA, or ISO 27001", score: 2 }, { label: "FedRAMP or multiple frameworks", score: 3 }] },
  { q: "Are you aware of all certificates in your environment, including shadow IT?", options: [{ label: "Yes, we have full visibility", score: 0 }, { label: "Mostly, but some gaps", score: 2 }, { label: "No — likely have unknown certs", score: 3 }, { label: "We've never audited for shadow certs", score: 4 }] },
];
              ...
              {q.options.map((opt, oi) => {
                ...
                return (
                  <div key={oi} onClick={() => setAnswers(prev => ({ ...prev, [qi]: opt.score, [`_${qi}`]: oi }))}
                    style={{ padding: "12px 16px", borderRadius: 6, cursor: "pointer", fontSize: 13, border: `1.5px solid ${sel ? C.teal : C.greyBorder}`, background: sel ? `${C.teal}12` : C.white, color: sel ? C.navy : C.muted, fontWeight: sel ? 600 : 400, transition: "all 0.15s" }}>
                    {opt.label}
                  </div>
                );
              })}
          ...
          <div style={{ background: C.white, border: `1px solid ${C.greyBorder}`, borderRadius: 8, padding: 40, marginTop: 8, textAlign: "center" }}>
            ...
            <div style={{ display: "flex", gap: 16, justifyContent: "center", flexWrap: "wrap" }}>
              <a href="https://www.linkedin.com/in/melizzabedrossi/" target="_blank" rel="noopener noreferrer"
                style={{ display: "inline-flex", alignItems: "center", gap: 8, background: "#0A66C2", color: C.white, padding: "13px 24px", borderRadius: 6, fontSize: 13, fontWeight: 700, textDecoration: "none" }}>
              ...
              <a href="sms:8018222706" style={{ display: "inline-flex", alignItems: "center", gap: 8, background: C.teal, color: C.white, padding: "13px 24px", borderRadius: 6, fontSize: 13, fontWeight: 700, textDecoration: "none" }}>
                📱 Text 801-822-2706
              </a>
            </div>
          </div>
    ...
    <section style={{ background: C.white, padding: "72px 32px", borderBottom: `1px solid ${C.greyBorder}` }}>
      <div style={{ maxWidth: 1000, margin: "0 auto" }}>
        <div style={{ marginBottom: 48 }}>
          ...
          <h2 style={{ fontFamily: "'DM Serif Display', serif", fontSize: 38, fontWeight: 400, color: C.navy, letterSpacing: "-0.02em" }}>What is your environment worth to manage?</h2>
          <p style={{ fontSize: 14, color: C.muted, marginTop: 10, lineHeight: 1.7, maxWidth: 600 }}>Tell us about your environment. We'll show you how your current vendor costs compare to alternatives — and what your renewal burden looks like as deadlines hit.</p>
        </div>
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 24 }}>
          <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
            <div style={{ background: C.grey, borderRadius: 8, padding: 24, border: `1px solid ${C.greyBorder}` }}>
              <label style={{ fontSize: 11, fontWeight: 700, color: C.muted, textTransform: "uppercase", letterSpacing: "0.12em", display: "block", marginBottom: 12 }}>Certificates Managed: <span style={{ color: C.navy, fontFamily: "'IBM Plex Mono', monospace" }}>{certCount}</span></label>
              <input type="range" min={5} max={2000} value={certCount} onChange={e => setCertCount(parseInt(e.target.value))} style={{ width: "100%", accentColor: C.teal }} />
              <div style={{ display: "flex", justifyContent: "space-between", fontSize: 10, color: C.muted, marginTop: 4 }}><span>5</span><span>2,000+</span></div>
            </div>
            <div style={{ background: C.grey, borderRadius: 8, padding: 24, border: `1px solid ${C.greyBorder}` }}>
              <label style={{ fontSize: 11, fontWeight: 700, color: C.muted, textTransform: "uppercase", letterSpacing: "0.12em", display: "block", marginBottom: 12 }}>Team Members Managing Certs: <span style={{ color: C.navy, fontFamily: "'IBM Plex Mono', monospace" }}>{teamSize}</span></label>
              <input type="range" min={1} max={20} value={teamSize} onChange={e => setTeamSize(parseInt(e.target.value))} style={{ width: "100%", accentColor: C.teal }} />
            </div>
            <div style={{ background: C.grey, borderRadius: 8, padding: 24, border: `1px solid ${C.greyBorder}` }}>
              ...
              <select value={vendor} onChange={e => setVendor(e.target.value)} style={{ width: "100%", padding: "11px 14px", border: `1.5px solid ${C.greyBorder}`, borderRadius: 6, fontSize: 13, background: C.white, color: C.navy, outline: "none" }}>
              ...
              <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
                {["On-Premises", "AWS", "Azure", "GCP", "Hybrid", "Edge / CDN"].map(env => (
                  <div key={env} onClick={() => toggleArr(environments, setEnvironments, env)}
                    style={{ padding: "7px 14px", borderRadius: 20, fontSize: 12, cursor: "pointer", fontWeight: 500, transition: "all 0.15s", background: environments.includes(env) ? C.teal : C.white, color: environments.includes(env) ? C.white : C.muted, border: `1.5px solid ${environments.includes(env) ? C.teal : C.greyBorder}` }}>{env}</div>
                ))}
              </div>
            </div>
            <div style={{ background: C.grey, borderRadius: 8, padding: 24, border: `1px solid ${C.greyBorder}` }}>
              ...
              <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
                {["PCI DSS", "HIPAA", "FedRAMP", "SOC 2", "ISO 27001", "NIST"].map(fw => (
                  <div key={fw} onClick={() => toggleArr(compliance, setCompliance, fw)}
                    style={{ padding: "7px 14px", borderRadius: 20, fontSize: 12, cursor: "pointer", fontWeight: 500, transition: "all 0.15s", background: compliance.includes(fw) ? C.navy : C.white, color: compliance.includes(fw) ? C.white : C.muted, border: `1.5px solid ${compliance.includes(fw) ? C.navy : C.greyBorder}` }}>{fw}</div>
                ))}
              </div>
            </div>
          </div>
          <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
            <div style={{ background: C.grey, borderRadius: 8, padding: 24, border: `1px solid ${C.greyBorder}` }}>
              ...
              <div style={{ marginBottom: 12 }}>
                ...
                <div style={{ fontFamily: "'IBM Plex Mono', monospace", fontSize: 28, fontWeight: 700, color: C.navy }}>{currentCost === 0 ? "No direct cost*" : `$${currentCost.toLocaleString()}/yr`}</div>
            ...
            <div style={{ background: C.navy, borderRadius: 8, padding: 24 }}>
              ...
              <div style={{ fontSize: 12, color: "rgba(255,255,255,0.6)", marginBottom: 20, lineHeight: 1.6 }}>Connect with Melizza Bedrossi, Chief Partnerships & Alliances Officer at SideChannel, to get a custom Enclave quote based on your environment.</div>
              <a href="https://www.linkedin.com/in/melizzabedrossi/" target="_blank" rel="noopener noreferrer"
                style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 10, background: "#0A66C2", color: C.white, padding: "13px 20px", borderRadius: 6, fontSize: 13, fontWeight: 700, textDecoration: "none" }}>
                <svg width="18" height="18" viewBox="0 0 24 24" fill="white"><path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433c-1.144 0-2.063-.926-2.063-2.065 0-1.138.92-2.063 2.063-2.063 1.14 0 2.064.925 2.064 2.063 0 1.139-.925 2.065-2.064 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z"/></svg>
                Connect on LinkedIn
              </a>
              <div style={{ fontSize: 11, color: "rgba(255,255,255,0.35)", textAlign: "center", marginTop: 12 }}>Or text <span style={{ color: C.green, fontWeight: 600 }}>801-822-2706</span> for a faster response</div>
            </div>
          </div>
        </div>
      </div>
  ...
  return (
    <section style={{ background: C.navy, padding: "72px 32px" }}>
      <div style={{ maxWidth: 700, margin: "0 auto", textAlign: "center" }}>
        ...
        <h2 style={{ fontFamily: "'DM Serif Display', serif", fontSize: 42, fontWeight: 400, color: C.white, letterSpacing: "-0.02em", marginBottom: 16 }}>Ready to see Enclave in action?</h2>
        <p style={{ fontSize: 15, color: "rgba(255,255,255,0.6)", lineHeight: 1.75, marginBottom: 40 }}>Text us directly. A SideChannel CLM specialist will respond within one business hour with demo availability or a custom pricing quote for your environment.</p>
        <a href="sms:8018222706?body=Hi, I'd like to request a demo of Enclave CLM" style={{ display: "inline-flex", alignItems: "center", gap: 12, background: C.teal, color: C.white, padding: "18px 40px", fontSize: 16, fontWeight: 700, borderRadius: 6, textDecoration: "none", letterSpacing: "0.02em", boxShadow: "0 8px 32px rgba(13, 158, 143, 0.4)" }}>
          📱 Text 801-822-2706
        </a>
        <div style={{ marginTop: 16, fontSize: 12, color: "rgba(255,255,255,0.4)" }}>Standard messaging rates apply · Mon–Fri 8am–6pm MT</div>
      </div>
    </section>
  );
}
...
export default function App() {
  const refs = { Deadlines: useRef(null), Scorecard: useRef(null), Calculator: useRef(null), Pricing: useRef(null) };
  const scrollTo = (section) => refs[section]?.current?.scrollIntoView({ behavior: "smooth", block: "start" });
  return (
